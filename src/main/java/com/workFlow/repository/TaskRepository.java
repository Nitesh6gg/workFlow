...
public interface TaskRepository extends JpaRepository<Task,Integer> {
    ...
    @Query(value = "SELECT t.`taskId`,t.`description`,t.`startDate`,t.`dueDate`,t.`priority`,t.`status`,t.`createdBy`,p.`projectName`,u.`username` FROM `task` t JOIN `project` p ON t.`projectId`=p.`projectId` JOIN `user` u ON u.`userId`=t.`assignUserId` WHERE t.`createdBy`=?",nativeQuery = true)
