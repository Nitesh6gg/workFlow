...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query(value ="SELECT u.`userId`,u.`username`,p.`title`FROM `user` u JOIN userrole r ON u.`userId`=r.`userId` JOIN `department` d ON r.`departmentId`=d.`id` JOIN `position` p ON r.`positionId`=p.`positionId` WHERE d.`id`=? AND p.`title`='Manager'",nativeQuery = true)
